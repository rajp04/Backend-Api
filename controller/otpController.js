...
exports.verify = async (req, res) => {
    try {
        ...
        if (Type == "auth_user") {
            const { _id, firstName, lastName, email, otp, otp_expires } = await User.findOne({ phno: Phone })
        ...
        if (Type == "auth_vendor") {
            const { _id, firstName, lastName, email, otp, otp_expires } = await Vender.findOne({ phno: Phone })
            const time = new Date().toLocaleString()
            // console.log(time ,vendor.otp_expires)
            if (otp_expires >= time) {
                if (Otp == otp) {
